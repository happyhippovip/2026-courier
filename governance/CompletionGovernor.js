...
class CompletionGovernor {
  ...
  evaluateTerminalRequest(request) {
    if (!request || typeof request !== 'object') {
      return { admitted: false, terminal: false, mission_status: 'ACTIVE_CONTINUUM', reason: 'INVALID_REQUEST' };
    }
    if (request.type === 'HUMAN_COMMAND' && (request.command === 'TERMINATE' || request.command === 'STOP')) {
      return { admitted: true, terminal: true, mission_status: 'TERMINAL_HUMAN_COMMAND', reason: request.reason || 'Terminated by explicit human authority' };
    }
    if (request.type === 'UNRECOVERABLE_FATAL_ERROR') {
      return { admitted: true, terminal: true, mission_status: 'TERMINAL_FATAL_ERROR', reason: request.error || 'Unrecoverable fatal system error' };
    }
    return {
      admitted: false,
      terminal: false,
      mission_status: 'ACTIVE_CONTINUUM',
      reason: `REJECTED: Only explicit human command or unrecoverable fatal error transitions mission to TERMINAL. Received: ${request.type || request.command || 'UNKNOWN'}`
    };
  }
}
