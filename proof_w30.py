...
def run_proof():
    ...
    try:
        req = urllib.request.Request("http://127.0.0.1:8080/workers/register",
                                    data=json.dumps({"worker_id": "w30-worker", "capabilities": ["windows"]}).encode(),
        ...
        def claim():
            req = urllib.request.Request("http://127.0.0.1:8080/tasks/claim",
                                         data=json.dumps({"worker_id": "w30-worker"}).encode(),
                                         headers={"Authorization": "Bearer test-secret-key", "Content-Type": "application/json"},
                                         method="POST")
            res = json.loads(urllib.request.urlopen(req).read())
            return res.get("task")
            
        def provider_wait(task):
            req = urllib.request.Request(f"http://127.0.0.1:8080/tasks/{task['task_id']}/provider_wait",
                                         data=json.dumps({"worker_id": "w30-worker", "wait_type": "WAITING_PROVIDER", "reason": "Rate limited"}).encode(),
                                         headers={"Authorization": "Bearer test-secret-key", "Content-Type": "application/json"},
                                         method="POST")
            urllib.request.urlopen(req)
        ...
        # Now t1 is WAITING_PROVIDER, but B should still be claimable.
        t2 = claim()
        ...
        # Now there should be NO tasks available, because t1 is in backoff (WAITING_PROVIDER)
