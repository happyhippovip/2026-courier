    ...
    def handle_api_state(self):
        ...
        if autonomy_dir.exists():
            ...
            if hb_file.exists():
                ...
                endurance_188g_data["is_running"] = hb.get("status") in ("RUNNING", "WAITING", "CONNECTED")
        ...
        task_board_items = []
        if queue_dir.exists():
            for qf in sorted(queue_dir.glob("*.json"), key=lambda p: p.name, reverse=True)[:100]:
                qd = load_json_safe(qf)
        ...
        session_view = {
            ...
            "current_phase": "EXECUTION" if autonomy_runtime_data.get("status") == "RUNNING" else ("GATE_PAUSE" if "GATE" in str(autonomy_runtime_data.get("status", "")) else "AVAILABLE"),
            ...
            "active_workers": [t.get("owner_agent") for t in task_board_items if t.get("status") == "RUNNING"] or [],
        ...
        execution_workers_def = {
            "worker-google": {
                ...
                "provider": "GOOGLE_PRO",
                "state": "RUNNING" if any(t.get("provider") == "GOOGLE_PRO" and t.get("status") in ("RUNNING", "RUNNING") for t in task_board_items) else "AVAILABLE",
                "task": next((t.get("title") for t in task_board_items if t.get("provider") == "GOOGLE_PRO" and t.get("status") in ("RUNNING", "RUNNING")), "Standby"),
            ...
            "worker-codex": {
                ...
                "provider": "CODEX",
                "state": "RUNNING" if any(t.get("provider") == "CODEX" and t.get("status") in ("RUNNING", "RUNNING") for t in task_board_items) else "AVAILABLE",
                "task": next((t.get("title") for t in task_board_items if t.get("provider") == "CODEX" and t.get("status") in ("RUNNING", "RUNNING")), "Standby"),
                "heavy_job": False,
