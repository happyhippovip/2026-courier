...
def validate_guard(guard: Any) -> None:
    ...
    if guard["transition_state"] == "CANONICAL_ACCEPTED":
        ...
        for item in evidence:
            if item["validity"] == "VALID" and item["evidence_sha"] == guard["binding"]["current_sha"] and item["runtime_binding"] == guard["binding"]["runtime_identity"]:
                if item["source_type"] == "MACHINE_ARTIFACT":
                    has_machine = True
                elif item["source_type"] in ("GITHUB_PULL_REQUEST", "GITHUB_COMMIT", "GITHUB_ISSUE_STATE"):
                    has_independent = True
        if not has_machine:
            raise LedgerError("CANONICAL_ACCEPTED requires a valid bound machine artifact")
        if not has_independent:
            raise LedgerError("CANONICAL_ACCEPTED requires independent verification bound to the exact candidate SHA")
    ...
    with writer_lock(path, timeout):
        ...
        if unproven or record.get("STATUS") in ("READY", "WAITING_PROVIDER", "DISPATCHED", "RUNNING"):
            ...
            if record.get("STATUS") == "CLEAN_IDLE":
                record["STATUS"] = "READY"
        elif not unproven and has_physical_proof and has_independent_proof:
