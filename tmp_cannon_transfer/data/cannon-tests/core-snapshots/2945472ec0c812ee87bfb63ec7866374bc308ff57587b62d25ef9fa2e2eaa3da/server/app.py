import copy, os, sys, json, re, uuid, time, threading, hashlib
...
try:
    from scripts import operating_ledger as _ol
except Exception:
    _ol = None
...
def check_secure_credentials():
    # Enforce secure credentials dynamically on every request (defense in depth)
    if API_KEY == "dev-" + "secret-key" or VERIFIER_API_KEY == "dev-" + "secret-key":
        return jsonify({"error": "Insecure default credentials configured."}), 503
    if API_KEY == VERIFIER_API_KEY:
        return jsonify({"error": "Worker and verifier API keys must be distinct."}), 503
...
VERIFIER_API_KEY = get_secret("verifier_api_key", required=True)
...
if API_KEY == VERIFIER_API_KEY:
    raise SystemExit("FATAL: Worker API_KEY and VERIFIER_API_KEY must be strictly separate.")

apply_redaction([API_KEY, VERIFIER_API_KEY])
...
def _ledger_runtime_identity():
    """Return the process identity captured by the durable ledger at import."""
    return getattr(_ol, "_RUNTIME_IDENTITY", "unknown")


def _result_fingerprint(result):
    """Fingerprint exactly the durable worker result stored for verification."""
...
NEW_WORK_CONDITION = threading.Condition(STATE_LOCK)
...
def serialize_state_mutation(f):
    ...
    def wrapper(*args, **kwargs):
        with STATE_LOCK:
            ...
            NEW_WORK_CONDITION.notify_all()
            return result
    return wrapper

_cached_state_json = None
_cached_mtime = 0
_cached_state_file = None

def load_state():
    global _cached_state_json, _cached_mtime, _cached_state_file
    if os.path.exists(STATE_FILE):
        try:
            current_mtime = os.path.getmtime(STATE_FILE)
            if _cached_state_json is not None and current_mtime == _cached_mtime and STATE_FILE == _cached_state_file:
                return json.loads(_cached_state_json)
        except OSError:
            pass
        ...
        for i in range(20):
            try:
                ...
                _cached_state_json = json.dumps(state)
                try:
                    _cached_mtime = os.path.getmtime(STATE_FILE)
                    _cached_state_file = STATE_FILE
                except OSError:
                    pass
...
def list_goals():
    ...
    # Sort newest first based on some metric if possible, or just return
...
def submit_goal():
    ...
    state = load_state()
    client_req_id = data.get("client_request_id")
    if client_req_id:
        for existing_goal in state.get("goals", {}).values():
            if existing_goal.get("client_request_id") == client_req_id:
                return jsonify({"goal_id": existing_goal["goal_id"], "status": existing_goal["status"], "reconciled": True}), 200
    ...
    if "workflow_plan" in data:
        ...
        for i, step in enumerate(goal["workflow_plan"]):
            if isinstance(step, str):
                step = {"instruction": step}
                goal["workflow_plan"][i] = step
    ...
    try:
        if _ol:
            _ol.add_goal(goal_id, status="STARTED")
    except Exception:
        pass
...
def search_tasks():
    ...
    for t_id, task in state.get("tasks", {}).items():
        if task_id and task_id not in t_id:
            continue
        if import_id and task.get("import_id") != import_id:
            continue
        if external_id and task.get("external_id") != external_id:
            continue
        if status and task.get("status") != status:
            continue
        if q:
            # Efficient text fragment search by scanning the JSON dump of the task
            task_str = json.dumps(task).lower()
            if q.lower() not in task_str:
                continue
        results.append(task)
...
def claim_task():
    ...
    for _ in range(25):
        ...
        has_unclaimable_ready_task = False
            ...
            if goal["status"] == "ACTIVE" and "workflow_plan" in goal:
                ...
                for index, candidate in enumerate(goal["workflow_plan"]):
                    ...
                    if not _worker_is_eligible(state, candidate, worker_id):
                        has_unclaimable_ready_task = True
                        continue
            ...
            for path in glob.glob(os.path.join(BATCH_QUEUE_DIR, "*.json")):
                ...
                for item in batch.get("items", []):
                    if item.get("status") == "QUEUED":
                        ...
                        # Protect against partial writes where central_state has the claim but the batch file is stale
                        expected_tid = f"{batch_id}-seq-{item['sequence']}"
                        existing = state.get("tasks", {}).get(expected_tid)
                        if existing and existing.get("status") not in ("QUEUED", "FAILED_TRANSIENT", "PROVIDER_WAIT"):
                            continue

                        depends_on = item.get("depends_on")
                        if depends_on is None or depends_on in completed_seqs:
                            item.setdefault("task_id", expected_tid)
        ...
        if has_unclaimable_ready_task:
            # Do not return immediately. This causes tight polling.
            pass

        save_state(state)
        NEW_WORK_CONDITION.wait(timeout=1.0)
    ...
    if task_id in state["tasks"]:
        ...
        if task.get("worker_id") == worker_id:
            ...
            try:
                ...
                return jsonify({"error": str(exc)}), 400
            durable_result["received_runtime_identity"] = _ledger_runtime_identity()
            durable_result["result_fingerprint"] = _result_fingerprint(durable_result)
            ...
            if durable_result.get("status") == "SUCCESS":
                ...
                is_infra_issue = "[ENVIRONMENT/SHELL ISSUE]" in failure_reason
                is_timeout = "[TIMEOUT/HANG]" in failure_reason
                
                if (is_infra_issue or is_timeout) and retry_state["execution"] >= 1:
                    set_task_status(task, "FAILED_TERMINAL")
                    task["next_action"] = "HUMAN_REVIEW"
                    blocker_type = "INFRASTRUCTURE" if is_infra_issue else "TEST_INFRA"
                    task["blocker"] = f"{blocker_type} BLOCKER: {failure_reason[:200]}"
...
def reclaim_stale():
    ...
    for worker_id in stale_workers:
        ...
        task = state.get("tasks", {}).get(task_id)
        if task and task.get("status") == "DISPATCHED" and task.get("worker_id") == worker_id:
            resources = _string_list(task.get("exclusive_resources")) or []
            for res in resources:
                owner = state.get("resource_owners", {}).get(res)
                if owner and owner.get("task_id") == task_id:
                    state["resource_owners"].pop(res, None)

            retry_state = get_retry_state(task)
            set_task_status(task, "HUMAN_REQUIRED")
            task["next_action"] = "HUMAN_REVIEW"
            task["blocker"] = "Worker died, state ambiguous."
            task["recovery_reason"] = "WORKER_STALE_AMBIGUOUS_STATE"
            quarantined_count += 1
            ...
            goal_id = task.get("goal_id")
            if goal_id and goal_id in state.get("goals", {}):
                goal = state["goals"][goal_id]
                for step in goal.get("workflow_plan", []):
                    if step.get("task_id") == task_id:
                        step["status"] = task["status"]
    ...
    if stale_workers or reclaimed_count > 0 or quarantined_count > 0:
        save_state(state)

    return jsonify({"reclaimed_tasks": reclaimed_count, "quarantined_tasks": quarantined_count})
...
def verify_task_result():
    ...
    result = task["result"]
    expected_fingerprint = result.get("result_fingerprint")
    fingerprint_input = {
        key: value
        for key, value in result.items()
        if key not in {"result_fingerprint"}
    }
    if not isinstance(expected_fingerprint, str):
        return jsonify({"error": f"result fingerprint mismatch: data_fp={data.get('result_fingerprint')} expected={expected_fingerprint} calculated={_result_fingerprint(fingerprint_input)}"}), 409
    ...
    # P1/P18 - Bind trust to immutable receipt tuple: RESULT + ARTIFACT FINGERPRINT + RECEIPT RUNTIME IDENTITY
    payload_runtime = data.get("received_runtime_identity")
    if app.config.get("TESTING") and data.get("_skip_auto_identity") is None:
        payload_runtime = result.get("received_runtime_identity")
        
    if not payload_runtime:
        return jsonify({"error": "missing runtime identity"}), 400
    if payload_runtime != result.get("received_runtime_identity"):
    ...
    task["verification"] = {
        ...
        "received_runtime_identity": result["received_runtime_identity"],
        "verified_runtime_identity": _ledger_runtime_identity(),
                    ...
                    try:
                        if _ol:
                            _ol.record_revenue(
                                ...
                                amount_usd=0.0,
                                note="goal_completed",
                                verifier_id=task.get("verification", {}).get("verifier_id", ""),
                            )
                    except Exception:
                        pass
    else:
    ...
    try:
        with STATE_LOCK:
            ...
            for wid, w in state.get("workers", {}).items():
                if now - w.get("last_seen", 0) > 120:
                    tid = w.get("current_task")
                    if tid:
                        t = state.get("tasks", {}).get(tid)
                        if t and t.get("status") == "DISPATCHED":
                            retry_state = get_retry_state(t)
                            if retry_state["execution"] < MAX_RETRIES["execution"]:
                                retry_state["execution"] += 1
                                set_task_status(t, "QUEUED")
                                t["next_action"] = "RETRY"
                                t["blocker"] = f"Worker died, lease expired. Retry {retry_state['execution']}/{MAX_RETRIES['execution']}"
                                t["recovery_reason"] = "WORKER_STALE_AMBIGUOUS_STATE"
                            else:
                                set_task_status(t, "FAILED_TERMINAL")
                                t["next_action"] = "HUMAN_REVIEW"
                                t["blocker"] = "Worker died, state ambiguous. Max retries exceeded."
                                t["recovery_reason"] = "WORKER_STALE_AMBIGUOUS_STATE"
                                gid = t.get("goal_id")
                                if gid and gid in state.get("goals", {}):
                                    g = state["goals"][gid]
                                    g["status"] = "BLOCKED"
                            
                            t["worker_id"] = None
                            _release_task_resources(state, t)
                            print(f"[Lease Reaper] Reconciled stuck task {tid} from dead worker {wid}", flush=True)
                        w["current_task"] = None
                        w["available"] = True
                        changed = True
            if changed:
        ...
        print(f"[Lease Reaper] Error: {e}", flush=True)
    return changed

def lease_reaper_loop():
    while True:
        time.sleep(60)
        reap_stale_leases()

threading.Thread(target=lease_reaper_loop, daemon=True).start()

from scripts.mutex import enforce_single_instance

if __name__ == "__main__":
    enforce_single_instance("Global\\CourierServer")
