...
async function refresh(){
 ...
 try{
  ...
  const counts={'Queue':c.QUEUED||0,'Läuft':s.active_lanes||0,'Wartet':c.RESULT_RECEIVED||0,'Blockiert':lanes.filter(l=>['UNKNOWN','ERROR'].includes(l.phase)).length,'Erledigt':m.DONE||0,'Übrig':Math.max(0,(data.session.count||0)-(m.DONE||0))};
  ...
  $('run-info').textContent=data.session.count?`${runLabel}: ${data.session.count} Aufgaben · ${data.session.mode}.`:'Noch kein Lauf gestartet.';
  if(data.helper_active)timer=setTimeout(refresh,1000);
 }catch(e){$('notice').textContent=e.message;}
}
async function act(action){try{const r=await fetch('/api/cannon/action',{method:'POST',headers:{'Content-Type':'application/json','X-Symphony-Token':token},body:JSON.stringify({action,mode:$('mode').value,count:Number($('count').value)})});const d=await r.json();if(!r.ok)throw Error(d.error);await refresh();}catch(e){$('notice').textContent=e.message;}}
