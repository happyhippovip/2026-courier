...
def run_proof():
    ...
    try:
        ...
        register_worker("w31-worker1")
        register_worker("w31-worker2")

        print("Submitting goal with 10 tasks...")
        plan = [{"task_id": f"T-W31-{i}", "instruction": f"Do step {i}", "target_agent": "windows"} for i in range(10)]
        
        req = urllib.request.Request("http://127.0.0.1:8080/goals", 
                                     data=json.dumps({"goal_text": "Verify W31", "workflow_plan": plan, "terminal": True}).encode(), 
                                     headers={"Authorization": "Bearer test-secret-key", "Content-Type": "application/json"},
                                     method="POST")
        urllib.request.urlopen(req)
        ...
        def verify_result(task):
            ...
            verify_data = {
                "task_id": task["task_id"],
                "verifier_id": "w31-verifier",
                "result_id": f"res-{task['task_id']}",
                "verdict": "PASS",
                "artifacts": [{"path": filename, "sha256": digest}],
                "received_runtime_identity": task.get("server_binding", "simulated_run")
            }
        ...
        t1 = threading.Thread(target=worker_loop, args=("w31-worker1",))
        t2 = threading.Thread(target=worker_loop, args=("w31-worker2",))
        ...
        if len(completed_tasks) != 10:
            sys.exit(f"Expected 10 tasks completed, got {len(completed_tasks)}")
            
        print("PASS_W31: 10-TASK ZERO-TOUCH ACCEPTANCE")
