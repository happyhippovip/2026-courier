...
debug_code = """
                            if not receipt or receipt.get("verdict") != "PASS" or \\
                               ...
                               receipt.get("goal_id") != bundle.get("record", {}).get("GOAL"):
                                logger.error(f"DEBUG REASON: {not receipt} {receipt.get('verdict') != 'PASS'} {receipt.get('result_sha256') != e.get('result_sha256')} {receipt.get('producer_principal') != e.get('producer_id')} {receipt.get('verifier_principal') != e.get('verifier_id')} {receipt.get('binding', {}).get('sha') != e.get('evidence_sha')} {receipt.get('binding', {}).get('runtime') != e.get('runtime_binding')} {receipt.get('goal_id') != bundle.get('record', {}).get('GOAL')}")
