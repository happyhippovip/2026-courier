...
import re
content = re.sub(r'        guard_changed = guard != bundle\.get\("acceptance_guard"\)\n        if guard_changed:\n            # Enforce independent producer/verifier boundary.*?raise LedgerError\("evidence produced by the acceptance decision path itself"\)', new_logic.strip(), content, flags=re.DOTALL)
