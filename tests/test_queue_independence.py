...
def test_queue_independence_a_waits_b_executes_c_executes(tmp_path):
    ...
    ledger_path = setup_ledger(
        tmp_path,
            unproven_edges=['EXTERNAL_PUBLICATION - SAFE_AUTOMATABLE_PREPARATION', 'PUBLIC DEPLOYMENT - SAFE_AUTOMATABLE_PREPARATION', 'PUBLIC DEPLOYMENT - AUTHORIZED_MACHINE_ACTION'],
            blocker="NONE",
            proven_edges=['LEDGER/HANDOFF', 'PR41 ACCEPTANCE', 'RELEASE - SAFE_AUTOMATABLE_PREPARATION', 'RELEASE - AUTHORIZED_MACHINE_ACTION', 'RELEASE - IRREVERSIBLE_HUMAN_ACTION', 'PUBLIC DEPLOYMENT - IRREVERSIBLE_HUMAN_ACTION', 'PUBLICATION VERIFICATION', 'PILOT INTAKE - SAFE_AUTOMATABLE_PREPARATION', 'PILOT INTAKE - AUTHORIZED_MACHINE_ACTION', 'PILOT INTAKE - IRREVERSIBLE_HUMAN_ACTION', 'SALES PACKAGE', 'FIRST PILOT - SAFE_AUTOMATABLE_PREPARATION', 'FIRST PILOT - AUTHORIZED_MACHINE_ACTION', 'FIRST PILOT - IRREVERSIBLE_HUMAN_ACTION', 'PAYMENT ONLY WHEN ACTUALLY REQUIRED - SAFE_AUTOMATABLE_PREPARATION', 'PAYMENT ONLY WHEN ACTUALLY REQUIRED - AUTHORIZED_MACHINE_ACTION', 'PAYMENT ONLY WHEN ACTUALLY REQUIRED - IRREVERSIBLE_HUMAN_ACTION', 'POST-PILOT HARDENING', 'EXTERNAL_PUBLICATION - AUTHORIZED_MACHINE_ACTION', 'EXTERNAL_PUBLICATION - IRREVERSIBLE_HUMAN_ACTION', 'ONBOARD_FIRST_PILOT_CUSTOMER - SAFE_AUTOMATABLE_PREPARATION', 'ONBOARD_FIRST_PILOT_CUSTOMER - AUTHORIZED_MACHINE_ACTION', 'ONBOARD_FIRST_PILOT_CUSTOMER - IRREVERSIBLE_HUMAN_ACTION'],
    )
    ...
    print(res.stdout)
    print(res.stderr)
    assert "PUBLIC DEPLOYMENT - AUTHORIZED_MACHINE_ACTION" not in unproven
