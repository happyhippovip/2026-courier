...
class TestCourierFallback(unittest.TestCase):
    ...
    def test_transient_gemini_failure_deterministic_local_cheap_succeeds(self):
        ...
        def mocked_submit_task(worker_id, task, mission_id=""):
            ...
            # For the second call, let's actually use the real CLI1 adapter to prove it works
            return original_submit_task(worker_id, task, mission_id)

        self.dispatcher.submit_task = mocked_submit_task
        # We need verify_result to pass since CLI1 produces a valid envelope
        ...
        self.assertEqual(submit_calls, ["GEMINI", "CLI1"])
    ...
    def test_local_cheap_makes_zero_native_agy_calls(self, mock_agy):
        ...
        adapter = create_real_cli1_adapter(self.tmp.name)
    ...
    def test_local_cheap_generic_discovery_with_evidence(self):
        ...
        with tempfile.TemporaryDirectory() as tmp_dir:
            ...
            adapter = create_real_cli1_adapter(tmp_dir)
            res = adapter(envelope)
            self.assertEqual(res.get("result", {}).get("status"), "COMPLETED")
    ...
    def test_schema_invalid_fallback(self):
        mission_id = "test-fallback-schema"
        self.dispatcher.mission_queue.enqueue({
            "mission_id": mission_id,
            "status": "PENDING",
            "task": {
        ...
        def mocked_submit_task(worker_id, task, mission_id=""):
            ...
            if target == "GEMINI":
                self.dispatcher.last_result_status = "DISPATCH_EXCEPTION"
                return {"status": "FAIL_CLOSED", "reason": "MISSING_ACK_OR_DISPATCH_FAIL: DISPATCH_FAILED: GEMINI_NATIVE_AGY_FAILED: Model did not return schema-valid JSON payload"}
            else:
                self.dispatcher.last_result_status = "DISPATCH_EXCEPTION"
                return {"status": "FAIL_CLOSED", "reason": "MISSING_ACK_OR_DISPATCH_FAIL: DISPATCH_FAILED: CLI1_FAILED"}

        self.dispatcher.submit_task = mocked_submit_task
        result = self.dispatcher.process_next_mission("worker-3")
        print("RESULT IS:", result)
        self.assertEqual(submit_calls, ["GEMINI", "CLI1"])
        self.assertEqual(result["status"], "BLOCKED")

    def test_quota_no_fallback(self):
        mission_id = "test-fallback-quota"
        self.dispatcher.mission_queue.enqueue({
            "mission_id": mission_id,
            "status": "PENDING",
            "task": {
        ...
        def mocked_submit_task(worker_id, task, mission_id=""):
            ...
            if target == "GEMINI":
                self.dispatcher.last_result_status = "DISPATCH_EXCEPTION"
                return {"status": "FAIL_CLOSED", "reason": "MISSING_ACK_OR_DISPATCH_FAIL: DISPATCH_FAILED: GEMINI_NATIVE_AGY_FAILED: RESOURCE_EXHAUSTED"}
            else:
                self.dispatcher.last_result_status = "PENDING_VERIFY"
                return {"status": "EXECUTED", "task_hash": "hash3", "requires_verify": False}

        self.dispatcher.submit_task = mocked_submit_task
        result = self.dispatcher.process_next_mission("worker-4")
        self.assertEqual(submit_calls, ["GEMINI"])
        self.assertEqual(result["status"], "BLOCKED")
        self.assertIn("RESOURCE_EXHAUSTED", result["reason"])
