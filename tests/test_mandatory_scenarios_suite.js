...
// coreRoot eliminated in minimality pass
...
// Replaced coreRoot duplicate imports with canonical SupervisorPlane exports
...
{
  const tempDir = path.join(shadowRoot, 'scratch', 'test_s16_' + Date.now());
  fs.mkdirSync(tempDir, { recursive: true });
  const audit = new AuditLedger(path.join(tempDir, 'audit'));
  const leaseMgr = new ProcessLeaseManager(path.join(tempDir, 'leases'), audit);
  ...
  const recon = reconciler.reconcile({
    livePids: [],
    liveProcessMap: {},
    pendingDispatchTasks: [{ task_id: 'TASK-CRASHED' }]
  });
  const isUncertain = recon.execution_uncertain_tasks.some(t => t.task_id === 'TASK-CRASHED');
  assert(16, 'crash after result persistence but before verification', isUncertain, 'Mid-flight task fenced as EXECUTION_UNCERTAIN');
  fs.rmSync(tempDir, { recursive: true, force: true });
}

// 17. restart and deterministic reconciliation
{
  const tempDir = path.join(shadowRoot, 'scratch', 'test_s17_' + Date.now());
  fs.mkdirSync(tempDir, { recursive: true });
  const audit = new AuditLedger(path.join(tempDir, 'audit'));
  audit.recordEvent({ task_id: 'TASK-1', event_type: 'PROGRESS_EVIDENCE', decision: 'KEEP_RUNNING' });
  audit.recordEvent({ task_id: 'TASK-1', event_type: 'HEARTBEAT', decision: 'KEEP_RUNNING' });
  const events = audit.getEvents();
  assert(17, 'restart and deterministic reconciliation', events.length >= 2 && events.some(e => e.event_type === 'PROGRESS_EVIDENCE'), 'Identical replay across multiple process runs');
  fs.rmSync(tempDir, { recursive: true, force: true });
}
