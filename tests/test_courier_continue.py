...
def test_all_scopes_blocked_true_global_stop(tmp_path):
    ...
    res = subprocess.run([sys.executable, str(Path(__file__, timeout=60).parent.parent / "scripts" / "courier_continue.py"), "--run", "--once"], env=dict(os.environ, MOCK_LEDGER=str(ledger_path), MOCK_BRANCH="test-branch", MOCK_SHA="0000000000000000000000000000000000000000"), capture_output=True, text=True)
    assert res.returncode == 0
...
def test_capability_insufficient_cannot_claim(tmp_path):
    ...
    res = subprocess.run([sys.executable, str(runner, timeout=60), "--once"], env=env, capture_output=True, text=True)
...
def test_worker_loss_takeover_and_handoff(tmp_path):
    ...
    res1 = subprocess.run([sys.executable, str(runner, timeout=60)], env=env1, capture_output=True, text=True)
    ...
    res2 = subprocess.run([sys.executable, str(runner, timeout=60)], env=env2, capture_output=True, text=True)
...
def test_blocked_dependent_does_not_freeze_independent(tmp_path):
    ...
    repo_dir = Path(__file__).parent.parent.resolve()
    res = subprocess.run(
        [sys.executable, str(repo_dir / "scripts" / "courier_continue.py", timeout=60)],
