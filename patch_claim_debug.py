...
content = content.replace('def _worker_is_eligible(state, task, worker_id):', '''def _worker_is_eligible(state, task, worker_id):
    ...
    if not worker: print(f"DEBUG: no worker {worker_id}", flush=True); return False
    if not worker.get("available", False): print(f"DEBUG: worker {worker_id} not available", flush=True); return False
    if worker.get("current_task"): print(f"DEBUG: worker {worker_id} busy with {worker.get('current_task')}", flush=True); return False
    ...
    if time.time() <= state.get("provider_locks", {}).get(lock_key, 0):
        print(f"DEBUG: provider lock {lock_key} active", flush=True)
        return False
    ...
    if required_capabilities:
        if not set(required_capabilities).issubset(set(worker_capabilities)):
            print(f"DEBUG: cap mismatch req {required_capabilities} got {worker_capabilities}", flush=True)
            return False
    elif not _legacy_target_matches(task, worker):
        print(f"DEBUG: legacy mismatch for task target {task.get('target_agent')} vs worker caps {worker_capabilities}", flush=True)
        return False
