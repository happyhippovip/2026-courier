...
def main():
    ...
    os.environ["COURIER_API_KEY"] = token
    os.environ["COURIER_SERVER"] = server_url
    ...
    # Server app looks for STATE_FILE in app.py
    # We will just patch the app env variables if they exist, or let it use default.
    # Wait, server.app uses global STATE_FILE
    ...
    while time.time() - start_time < 30:
        goals = http_get(f"{server_url}/goals", token)
        g = goals.get(goal_id, {})
        status = g.get("status")
    ...
    goals2 = http_get(f"{server_url}/goals", token)
    g2 = goals2.get(goal_id, {})
