...
def status():
    ...
    with LOCK:
        ...
        elif st == 'IDLE': st = 'WAITING' if mac.get('waiting_for_work') else 'IDLE'
        elif is_active:
        ...
        mode = ov.get('laufart', 'UNENDLICH')
        count = ov.get('verbleibend', '∞')
...
def action(body):
    ...
    with LOCK:
        ...
        if action_type == 'START':
            ...
            # Unlimited-only product mode: client count/mode values are
            # ignored. DAUERLAUF always means RUN_TARGET=UNLIMITED and the
            # backend never receives a --limit flag.
            if JOB: JOB.close(); JOB = None
            if CHILD is not None and CHILD.stdin: CHILD.stdin.close()

            command = [sys.executable, str(MAC_ADAPTER), "dauerlauf", "--art", "unendlich", "--local-fake"]
        ...
        if action_type == 'RESUME':
            ...
            if not active:
                ...
                command = [sys.executable, str(MAC_ADAPTER), "supervise"]
                CHILD, JOB, _ = launch_gated(command, str(ROOT), dict(os.environ), stderr=subprocess.PIPE)
                def bounded_errors(child):
                    for chunk in iter(lambda: child.stderr.read(1024), b''):
                        pass
