...
# Extract from def create_real_windows_adapter(root): to return windows_worker
# and replace it.
...
# Replace in adapters_code
code = re.sub(
    r"def create_real_windows_adapter\(root\):.*?return windows_worker\n",
    new_adapter,
    code,
    flags=re.DOTALL
)
