...
def main():
    ...
    if not tasks:
        # tasks might be stored elsewhere or modified
        pass
        
    # We can fetch state file directly if needed!
    with open(server_state, "r") as f:
        state_data = json.load(f)
        goal_record = state_data.get("goals", {}).get(goal_id, {})
        tasks = goal_record.get("workflow_plan", [])
        print("Tasks in state:", json.dumps(tasks, indent=2))
    ...
    with open("REAL_WORKER_BINDING_PROOF.md", "w") as f:
        ...
        f.write(f"- Candidate SHA/tree: Current Working Tree\n")
        ...
        f.write(f"- final status: {t.get('status', 'N/A')} (Goal: {status2})\n")
